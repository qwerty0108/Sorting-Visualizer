...
def drawData(data, colorArray):
    ...
    for i, height in enumerate(normalised_data):
        ...
        y0 = canvas_height - height * 400             # we have multiplied 400 because we will normalise our values with one formula so that our data won't exceed our canvas
...
def StartAlgorithm():
    ...
    bubble_sort(data, drawData, 1/speedscale.get())
...
random_generate = Button(root, text="Generate", bg="#2DAE9A", font=("arial", 12, "italic bold"), relief=SUNKEN, activebackground="#05945B", activeforeground="white", bd=5,
...
start = Button(root, text="Start", bg="#2DAE9A", font=("arial", 12, "italic bold"), relief=SUNKEN, activebackground="#05945B", activeforeground="white", bd=5,
width=10, command=StartAlgorithm)
